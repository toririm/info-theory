...
この結果は、元ファイルが1文字あたり $8 "bit"$ で表されていたと仮定したときに、理想的な圧縮率が
$
  H(X) / 8 = 4.7053 / 8 = 0.588...
$
となる理論値と整合する。
...
通信容量は、講義資料と同様の議論により
$
  C &= attach(max, b: p(a_i)) overline(I)(X;Y) \
  &= attach(max, b: p(a_i)) H(Y) - H(q) \
  &= 1 - H(q)
$
いま $q = 0.01$ であるから
$
  C &= 1 + 0.01 log_2 0.01 + (1 - 0.01) log_2 (1 - 0.01)
    &= 0.9192068 dots
$

$star$ $m = 2$ のケース

符号語長 $n = 2^m - 1 = 3$, 情報記号長 $k = n - m = 1$

情報伝送速度は
$
  R = (log_2 M) / n = (log_2 2) / 3 = 1/3
$

ハミング符号において復号誤りをする場合は、誤りが1個を超えたとき、すなわち誤りが $2$ 個の場合と $3$ 個の場合である。

よって、復号誤り率は
$
  P_e
  &= attach(C, bl: 3, br: 2)dot q^2(1-q) + q^3 \
  &= 0.000927 + 0.000001 = 0.000928
$

通信路符号化定理より
$
  R - C = 1/3 - 0.9192068 = -0.58587dots < 0
$
より、この場合ハミング符合より小さい任意の復号誤り率で送信できる通信路符合が存在する。

$star$ $m = 3$ のケース

符号語長 $n = 2^m - 1 = 7$, 情報記号長 $k = n - m = 4$

情報伝送速度は
$
  R = (log_2 M) / n = (log_2 2) / 7 = 1/7
$

復号誤りをする場合は、誤りが $2$ から $7$ 個の場合である。

よって、復号誤り率は
$
  P_e
  &= sum_(i=2)^7 attach(C, bl: 7, br: i)dot q^i (1-q)^(7-i) \
  &tilde 0.0020310416
$

通信路符号化定理より
$
  R - C = 1/7 - 0.9192068 = -0.77634dots < 0
$
より、この場合ハミング符合より小さい任意の復号誤り率で送信できる通信路符合が存在する。

$star$ $m = 4$ のケース

符号語長 $n = 2^m - 1 = 15$, 情報記号長 $k = n - m = 11$

情報伝送速度は
$
  R = (log_2 M) / n = (log_2 2) / 15 = 1/15
$

復号誤りをする場合は、誤りが $2$ から $15$ 個の場合である。

よって、復号誤り率は
$
  P_e
  &= sum_(i=2)^15 attach(C, bl: 15, br: i)dot q^i (1-q)^(15-i) \
  &tilde 0.0096297734
$

通信路符号化定理より
$
  R - C = 1/15 - 0.9192068 = -0.85354dots < 0
$
より、この場合ハミング符合より小さい任意の復号誤り率で送信できる通信路符合が存在する。

$star$ $m = 5$ のケース

符号語長 $n = 2^m - 1 = 31$, 情報記号長 $k = n - m = 26$

情報伝送速度は
$
  R = (log_2 M) / n = (log_2 2) / 31 = 1/31
$

復号誤りをする場合は、誤りが $2$ から $31$ 個の場合である。

よって、復号誤り率は
$
  P_e &= sum_(i=2)^31 attach(C, bl: 31, br: i)dot q^i (1-q)^(31-i) \
  &tilde 0.0383895146
$

通信路符号化定理より
$
  R - C = 1/31 - 0.9192068 = -0.88694dots < 0
$
より、この場合ハミング符合より小さい任意の復号誤り率で送信できる通信路符合が存在する。

== 課題３

おそらく意図されていたものだろうが、ハミング距離の説明について、
長さ $3$ の二元符合の空間を考えることで、$3$ 次元の空間とマッピングすることができる。
このとき、ハミング距離は $3$ 次元空間におけるマンハッタン距離に対応する。

長さ $n$ の一般の場合の二元符合の空間については、（具体的な図の想像は難しいが）$n$ 次元の空間について同様の議論を考えれば、直感的な理解の助けになった。
